...
<x-ui.card title="Questions" :subtitle="$questionPromptLabel">
    <form method="POST" action="{{ route('admin.reading-question-groups.matching.questions.store', $group) }}" class="mb-5 space-y-3">
        @csrf
    ...
    <form id="matching-question-reorder-form" method="POST" action="{{ route('admin.reading-question-groups.matching.reorder', $group) }}">
        @csrf
    ...
    <div id="matching-question-sortable" class="space-y-3">
        @forelse ($questions as $question)
            ...
            <div data-question-item data-question-id="{{ $question->id }}" class="rounded-xl border border-neutral-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-900">
                <form method="POST" action="{{ route('admin.reading-questions.update', $question) }}" class="space-y-3">
                    @csrf
                    @method('PUT')
                    <div class="grid gap-3 md:grid-cols-6">
                ...
                <form method="POST" action="{{ route('admin.reading-questions.destroy', $question) }}" class="mt-2" onsubmit="return confirm('Delete question {{ $question->question_number }}?')">
                    @csrf
                    @method('DELETE')
