<x-ui.card title="Questions" subtitle="Edit, reorder, or delete short answer questions">
    <form id="short-answer-question-reorder-form" method="POST" action="{{ route('admin.reading-question-groups.short-answer-questions.reorder', $group) }}">
        @csrf
    ...
    <div id="short-answer-question-sortable" class="space-y-4">
        @forelse ($questions as $question)
            ...
            <div data-question-item data-question-id="{{ $question->id }}" class="rounded-xl border border-neutral-200 bg-white p-4 dark:border-neutral-700 dark:bg-neutral-900">
                <form method="POST" action="{{ route('admin.reading-short-answer-questions.update', $question) }}" class="space-y-3">
                    @csrf
                    @method('PUT')
                ...
                <form method="POST" action="{{ route('admin.reading-short-answer-questions.destroy', $question) }}" class="mt-2" onsubmit="return confirm('Delete question {{ $question->question_number }}?')">
                    @csrf
                    @method('DELETE')
