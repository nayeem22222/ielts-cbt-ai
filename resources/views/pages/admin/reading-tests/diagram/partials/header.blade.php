@props(['test', 'passage', 'group', 'type', 'labelCount' => 0, 'expectedCount' => 0])

<x-ui.card class="mb-6">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0 flex-1">
            <p class="text-xs font-semibold uppercase tracking-wide text-brand-600">Diagram Label Builder</p>
            ...
            <dl class="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                    <dt class="aa-muted">Expected Questions</dt>
                    <dd class="font-medium">{{ $expectedCount }}</dd>
                </div>
                <div>
                    <dt class="aa-muted">Labels Placed</dt>
                    <dd class="font-medium"><span x-text="labels.length">{{ $labelCount }}</span> / {{ $expectedCount }}</dd>
                </div>
            </dl>
        </div>
