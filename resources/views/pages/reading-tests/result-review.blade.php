<x-layouts.reading-exam :title="$test->title.' — Review'">
    @php
        $reviewState = [
            'parts' => $parts,
    ...
    <div
        x-data="readingTestResultReview(@js($reviewState))"
        x-init="init()"
    ...
    >
        <header class="reading-test-header z-30 flex h-14 shrink-0 items-center justify-between border-b border-neutral-200 bg-white px-4 shadow-sm">
            ...
            <a href="{{ route('reading-attempts.result', $attempt) }}"
        ...
        <div class="flex min-h-0 flex-1">
            ...
            >
                <template x-for="passage in passages" :key="'passage-'+passage.id">
                    <div x-show="activePassageId === passage.id" x-cloak class="p-6 lg:p-8">
                        <h2 class="mb-4 text-center text-xl font-bold text-neutral-900" x-text="'Part ' + passage.part_number + (passage.title ? ' — ' + passage.title : '')"></h2>
                        <div class="reading-passage-body font-serif text-[15px] leading-8 text-neutral-900" :data-passage-id="passage.id" x-html="passage.content_html"></div>
                    </div>
                </template>
            </section>
                        ...
                        <section class="rounded-2xl border border-neutral-200 bg-white shadow-sm">
                            ...
                            <div class="divide-y divide-neutral-100">
                                @foreach ($part['questions'] as $item)
                                    ...
                                    <article
                                        id="review-question-{{ $item['question_number'] }}"
                                    ...
                                    >
                                        <div class="flex flex-wrap items-start justify-between gap-3">
                                            <div>
                                                <p class="text-sm font-semibold text-neutral-900">
                                                    ...
                                                    <span class="font-normal text-neutral-500">· {{ $item['question_type_label'] ?? 'Reading' }}</span>
                                                </p>
                                                @if (! empty($item['reference_paragraph']))
                                                    <p class="mt-1 text-xs text-neutral-500">Reference: Paragraph {{ $item['reference_paragraph'] }}</p>
                                                @endif
                                            </div>
                                        ...
                                        @if (! empty($item['prompt']))
                                            <p class="mt-3 text-sm text-neutral-700">{{ $item['prompt'] }}</p>
                                        @endif
