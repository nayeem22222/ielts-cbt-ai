<template x-if="group.renderer === 'matching-information'">
    <section class="rounded-lg border border-neutral-300 bg-white">
        <div class="border-b border-neutral-200 px-4 py-3">
            <div class="flex items-start justify-between gap-4">
                ...
                <span class="text-xs font-semibold uppercase tracking-wide text-neutral-400">Matching Information</span>
            </div>
        </div>
            ...
            <table class="w-full min-w-[34rem] border-collapse text-sm">
                <thead>
                    <tr class="bg-neutral-50">
                        <th class="w-10 border-b border-r border-neutral-200 px-2 py-2 text-left"></th>
                        <th class="border-b border-r border-neutral-200 px-3 py-2 text-left font-semibold text-neutral-600">Information</th>
                        <template x-for="option in group.options" :key="'mi-head-'+option.label">
                            <th class="border-b border-neutral-200 px-3 py-2 text-center font-bold text-neutral-700" x-text="option.label"></th>
                        </template>
                        <th class="w-16 border-b border-l border-neutral-200 px-2 py-2"></th>
                    </tr>
                </thead>
                <tbody>
                    <template x-for="question in group.questions" :key="'mi-row-'+question.id">
                        <tr :id="'question-'+question.id" class="transition" :class="activeQuestionId === question.id ? 'bg-emerald-50/70' : 'bg-white'">
                            <td class="border-r border-t border-neutral-200 px-2 py-3 align-top font-bold text-neutral-700" x-text="question.number"></td>
                            ...
                            <template x-for="option in group.options" :key="'mi-cell-'+question.id+'-'+option.label">
