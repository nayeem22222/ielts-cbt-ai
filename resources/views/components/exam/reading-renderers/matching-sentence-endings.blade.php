<template x-if="group.renderer === 'matching-sentence-endings'">
    <section class="rounded-lg border border-neutral-300 bg-white">
        ...
        <div class="grid gap-4 p-4 xl:grid-cols-[1fr_18rem]">
            <div class="space-y-3">
                <template x-for="question in group.questions" :key="'ending-question-'+question.id">
                    <div :id="'question-'+question.id" class="rounded border p-3 transition" :class="activeQuestionId === question.id ? 'border-[#2D6A4F] bg-emerald-50/60' : 'border-neutral-200 bg-white'">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <template x-for="option in group.options" :key="'ending-pick-'+question.id+'-'+option.label">
                                <button
                                    type="button"
            ...
            <aside class="rounded border border-neutral-200 bg-neutral-50 p-3">
                <p class="mb-2 text-xs font-bold uppercase tracking-wide text-neutral-500">Sentence Endings</p>
                <div class="space-y-2">
                    <template x-for="option in group.options" :key="'ending-option-'+option.label">
