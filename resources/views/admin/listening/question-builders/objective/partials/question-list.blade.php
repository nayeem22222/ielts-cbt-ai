<x-ui.card title="Questions" subtitle="Drag to reorder">
    <form id="objective-question-reorder-form" method="POST" action="{{ route('admin.listening-question-groups.objective-questions.reorder', $group) }}">
        @csrf
    ...
    <div id="objective-question-sortable" class="space-y-4">
        @forelse ($questions as $question)
            ...
            <div data-question-item data-question-id="{{ $question->id }}" @class(['rounded-xl border p-4', 'border-amber-300 bg-amber-50/40 dark:border-amber-700' => $isDraft, 'border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-900' => ! $isDraft])>
                ...
                <form method="POST" action="{{ route('admin.listening-objective-questions.update', $question->id) }}" class="space-y-3">
                    @csrf
                    @method('PUT')
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                    <form method="POST" action="{{ route('admin.listening-objective-questions.destroy', $question->id) }}" onsubmit="return confirm('Delete question{{ $displayNumber ? ' '.$displayNumber : '' }}?')">
                        @csrf
                        @method('DELETE')
