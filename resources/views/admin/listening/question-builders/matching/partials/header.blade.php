<x-ui.card title="Question Group" padding="p-5" class="mb-6">
    <dl class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 text-sm">
        <div>
            <dt class="text-xs uppercase aa-muted">Listening Test</dt>
            <dd class="font-semibold text-neutral-900 dark:text-white">{{ $listeningTest->title }}</dd>
        </div>
        <div>
            <dt class="text-xs uppercase aa-muted">Section</dt>
            <dd class="font-semibold">{{ $section->title }} (Part {{ $section->section_number }})</dd>
        </div>
        <div>
...
@include('admin.listening.question-builders.partials.interaction-settings', ['group' => $group, 'mode' => 'matching'])
