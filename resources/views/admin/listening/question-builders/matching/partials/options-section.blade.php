<x-ui.card title="Options" :subtitle="$optionTextLabel">
    ...
    <form method="POST" action="{{ route('admin.listening-question-groups.matching.options.store', $group) }}" class="mb-5 grid gap-3 md:grid-cols-4">
        @csrf
    ...
    <form id="matching-option-reorder-form" method="POST" action="{{ route('admin.listening-question-groups.matching.reorder', $group) }}">
        @csrf
        <div data-option-ids>
    ...
    <div id="matching-option-sortable" class="space-y-2">
        @forelse ($options as $option)
            <div data-option-item data-option-id="{{ $option->id }}" class="rounded-xl border border-neutral-200 bg-neutral-50 p-3 dark:border-neutral-700 dark:bg-neutral-900/50">
                <form method="POST" action="{{ route('admin.listening-matching-options.update', [$group, $option->id]) }}" class="grid gap-2 md:grid-cols-12 md:items-end">
                    @csrf
                    @method('PUT')
                ...
                <form method="POST" action="{{ route('admin.listening-matching-options.destroy', [$group, $option->id]) }}" class="mt-2 flex justify-end" onsubmit="return confirm('Delete option {{ $option->option_key }}? Questions using it will need reassignment.')">
                    @csrf
                    @method('DELETE')
