<x-ui.card title="Add Short Answer Question" subtitle="Official IELTS style — question text with word limit rule">
    ...
    <form method="POST" action="{{ route('admin.listening-question-groups.short-answer-questions.store', $group) }}" class="space-y-4" x-data="{ answerRule: @js($settings['answer_rule']), customAnswerRule: @js($settings['custom_answer_rule'] ?? '') }">
        @csrf
