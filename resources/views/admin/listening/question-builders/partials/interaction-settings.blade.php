...
@php
    ...
    $interactionMode = ListeningGroupInteraction::mode($group);
    $allowReuse = ListeningGroupInteraction::allowReuse($group);
    $modes = $mode === 'completion'
        ? ListeningGroupInteraction::completionInteractionModes()
        : ListeningGroupInteraction::matchingInteractionModes();
    $showAllowReuse = $mode === 'matching';
@endphp

<x-ui.card title="Interaction Settings" class="mb-6">
    <form
        method="POST"
        action="{{ route('admin.listening-question-groups.interaction-settings.update', $group) }}"
