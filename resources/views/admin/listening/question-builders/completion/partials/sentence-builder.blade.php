    ...
    <div x-data="{ mode: 'manual' }" class="space-y-6">
        ...
        <div x-show="mode === 'manual'" class="grid gap-6 xl:grid-cols-2">
            <div>
                ...
                <form method="POST" action="{{ route('admin.listening-question-groups.completion-questions.store', $group) }}" class="grid gap-4 md:grid-cols-2">
                    @csrf
            ...
            <div>
                @include('admin.listening.question-builders.completion.partials.sentence-list', [
                    'group' => $group,
                    'questions' => $questions,
                ])
            </div>
