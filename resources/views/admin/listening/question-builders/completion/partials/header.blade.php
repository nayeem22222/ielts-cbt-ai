@props(['test', 'section', 'group', 'type', 'detectedCount' => 0, 'expectedCount' => 0])
    ...
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0 flex-1">
            ...
            <dl class="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div>
                    <dt class="aa-muted">Detected / Saved</dt>
                    <dd class="font-medium">
                        <span x-text="liveDetectedCount ?? {{ $detectedCount }}">{{ $detectedCount }}</span>
                        / {{ $expectedCount }}
                    </dd>
                </div>
            </dl>
...
@include('admin.listening.question-builders.partials.interaction-settings', ['group' => $group, 'mode' => 'completion'])
