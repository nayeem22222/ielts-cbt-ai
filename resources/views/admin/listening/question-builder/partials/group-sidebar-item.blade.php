    ...
    <div class="flex items-start gap-2">
        ...
        <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-1.5">
                ...
                <x-ui.badge :tone="$group->is_active ? 'green' : 'neutral'">{{ $group->is_active ? 'Active' : 'Inactive' }}</x-ui.badge>
            </div>
            ...
            <p class="mt-1 text-xs aa-muted">
                Q{{ $group->question_range_label }} · Questions {{ $group->question_count_label }} · Order {{ $group->display_order }}
            </p>

            <div class="mt-2 flex flex-wrap gap-1">
                <x-ui.button
                    href="{{ route('admin.listening.tests.builder.index', ['listeningTest' => $listeningTest, 'section' => $section->id, 'question_group' => $group->id]) }}"
                    size="sm"
                    :variant="$selected ? 'primary' : 'outline'"
                >Edit</x-ui.button>

                <x-ui.button
                    href="{{ \App\Support\Listening\ListeningQuestionBuilderRoutes::manageQuestionsUrl($group) }}"
                    size="sm"
                    variant="outline"
                ...
                @can('delete', $group)
                    <form
                        method="POST"
                        action="{{ route('admin.listening.tests.sections.groups.destroy', [$listeningTest, $section, $group]) }}"
                ...
                @unless ($isFirst)
                    <form method="POST" action="{{ route('admin.listening.tests.sections.groups.move-up', [$listeningTest, $section, $group]) }}">
                        @csrf
                ...
                @unless ($isLast)
                    <form method="POST" action="{{ route('admin.listening.tests.sections.groups.move-down', [$listeningTest, $section, $group]) }}">
                        @csrf
